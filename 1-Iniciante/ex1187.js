...
const operation_above_main_and_sec_diagonal = (mat, op) => {
    let sum = 0;
    let count = 0;
    for(let i = 0; i < 5; i++){
        for(let j = i + 1; j < 12 - (i + 1); j++){
            sum += mat[i][j];
            count++;
        }
    }
...
// output
console.log(operation_above_main_and_sec_diagonal(matrice, operation).toFixed(1));